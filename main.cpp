...
void createPlatform() {
  ...
  Eigen::MatrixXi tempF(platF.rows(), 3);
  tempF << platF.col(2), platF.col(1), platF.col(0);
  platF = tempF;
}

void updateMeshes(igl::opengl::glfw::Viewer &viewer) {
  Eigen::RowVector3d platColor;
  platColor << 0.8, 0.8, 0.8;
  Eigen::RowVector3d meshColor;
  meshColor << 0.8, 0.2, 0.2;
  {
    Eigen::MatrixXd currV(scene.meshes[0].origPositions.size() / 3, 3);
  ...
  for (int i = 0; i < scene.meshes.size(); i++) {
    Eigen::MatrixXd currV(scene.meshes[i].origPositions.size() / 3, 3);
  ...
  Eigen::MatrixXi constF;
  Eigen::MatrixXd constV, constC;

  Eigen::MatrixXd P1(scene.userConstraints.size(), 3);
  Eigen::MatrixXd P2(scene.userConstraints.size(), 3);
  for (int i = 0; i < scene.userConstraints.size(); i++) {
    std::cout << "scene.userConstraints[i].globalIndices:" << scene.userConstraints[i].globalIndices << std::endl;
  ...
  Eigen::MatrixXd cyndColors = Eigen::RowVector3d(1.0, 1.0, 0.0).replicate(P1.size(), 1);
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    cout
        << "Please provide path (argument 1), name of scene file (argument 2), and name of constraints file (argument 3)!"
        << endl;
    return 0;
  }
